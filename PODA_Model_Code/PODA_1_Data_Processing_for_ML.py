...
@author: hexx
THis code compile COVID pandemic, mobility data, demograhpic, policy data for Machine Learning (ML)
The file will be saved on local folder, and saved on PODA_Model_2020_MM_YY.npy
"""
import pandas as pd
import numpy as np
...
'''
Need to check YYG website to adjust the date difference between today and the latest YYG projection
...
Apple_File_Date = pd.to_datetime('today')-pd.DateOffset(days=Apple_Date_adjust)
Apple_File_Date = Apple_File_Date.strftime("%Y-%m-%d")
PODA_Model = {'Date': today,
              'ML_File_Date': today,
              'Apple_File_Date': Apple_File_Date,
              'YYG_File_Date': YYG_Date}

df_StateName_Code = pd.read_excel('./US_StateCode_List.xlsx', sheet_name='Sheet1', header=0)
...
df_Google = pd.read_csv('https://www.gstatic.com/covid19/mobility/Global_Mobility_Report.csv?cachebust=911a386b6c9c230f', header=0)
df_Google.to_csv('./Mobility Google-Apple/Google_download_'+today+'.csv')
...
googleMobilityStateLevel = googleMobilityData[(googleMobilityData['County Name'].isna()) & (googleMobilityData['State Name'].notna())]
...
population_File = './Population_County_Level.xlsx'
...
employee_File = './Unemployment.xlsx'
...
US_Confirmed = df_Confirmed_Ini.sum().drop(labels=['State Name', 'stateFIPS', 'countyFIPS', 'County Name', 'State Code']).rename("US Total Confirmed")
...
US_Death = df_Death_Ini.sum().drop(labels=['State Name', 'stateFIPS', 'countyFIPS', 'County Name', 'State Code']).rename("US Total Death")
...
for stateName in stateNameList:
    ...
    state_Confirmed = state_Confirmed.sum().drop(labels=['State Name', 'stateFIPS', 'countyFIPS', 'County Name', 'State Code']).rename("State Total Confirmed")
    ...
    state_Death = state_Death.sum().drop(labels=['State Name', 'stateFIPS', 'countyFIPS', 'County Name', 'State Code']).rename("State Total Death")
    ...
    df_Projection=df_Projection[df_Projection['date'] <= (pd.to_datetime('today')+pd.DateOffset(days=7))]
    ...
    apple_State.rename(columns = {apple_State.columns[0]: "Apple State" }, inplace = True)
    apple_State.index = pd.to_datetime(apple_State.index)
    apple_Mobility = pd.merge_asof(apple_US, apple_State, left_index=True,right_index=True)
    merged1 = pd.merge_asof(merged1, apple_Mobility, left_index=True,right_index=True)
...
all_Data.to_excel("./ML Files/State_Level_Data_forML_"+today+".xlsx")

# cut_off_date = '2020-04-24'
# all_Cut = all_Data[all_Data.index <= pd.to_datetime(cut_off_date)]
# all_Cut.to_excel("./ML Files/State_Level_Data_forML_"+cut_off_date+".xlsx")
